...
export default function WalletPage() {
  ...
  const cryptoTotal = cryptoAssets.reduce(
    (sum, asset) => sum + (asset.valueUSD || 0) * 5,
    0
  );
            ...
            <div className="flex gap-6">
              <div>
                ...
                <p className="text-xl font-semibold text-blue-400">
                  R${" "}
                  {cryptoTotal.toLocaleString("pt-BR", {
                    minimumFractionDigits: 2,
                  })}
                </p>
              ...
              <div>
                ...
                <p className="text-xl font-semibold text-amber-400">
                  R${" "}
                  {fiatTotal.toLocaleString("pt-BR", {
                    minimumFractionDigits: 2,
                  })}
                </p>
                <p className="text-xs text-zinc-500">
          ...
          {[
            {
              id: "all",
              label: "All Assets",
              count: cryptoAssets.length + fiatAssets.length,
            },
      ...
      <section className="mb-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            >
              <div
                className={`w-10 h-10 rounded-lg bg-${action.color}-600/20 flex items-center justify-center text-lg`}
              >
                {action.icon}
              </div>
          ...
          {(activeTab === "all" || activeTab === "crypto") &&
            cryptoAssets.map((asset) => (
              <AssetCard
                ...
                type="crypto"
                onClick={() =>
                  router.push(`/wallet/crypto?asset=${asset.symbol}`)
                }
              />
            ))}
          ...
          {(activeTab === "all" || activeTab === "fiat") &&
            fiatAssets.map((asset) => (
              <AssetCard
                ...
                type="fiat"
                onClick={() =>
                  router.push(`/wallet/fiat?asset=${asset.symbol}`)
                }
              />
            ))}
        </div>
